...
Format changed Go files with `gofmt`. Run `go test -race ./...` when a change can affect concurrent or shared behavior, and `go test -bench=. ./...` when parser or duplicate-detection work may affect performance.
